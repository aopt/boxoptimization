...
c(a(i,j)) = sqr(size(j-1)) * sum(ijk(i,j-1,k),count(k));
c(i,'sink') = sqr(size(lastj)) * sum(ijk(i,lastj,k),count(k));
...
equations
  nodbal       'node balance'
  calcoutflow  'calc y'
  numboxes     'we need an active node for box'
  obj          'objective'
;
...
calcoutflow(iext).. y(iext) =e= sum(a(iext,jext),x(a));

nodbal(iext).. sum(a(jext,iext), x(a)) + pinflow(iext) =e= y(iext);

numboxes.. sum(i,y(i)) =e= T;
...
abort$sum((iext,jext)$(abs[x.l(iext,jext)-round(x.l(iext,jext))]>1e-4),1)
